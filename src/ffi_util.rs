...
macro_rules! ffi_type {
    ($n:ident, $r:ident, $c:path, $d:path) => {
        ...
        impl ::ffi_util::ForeignType for $n {
            type CType = $c;
            type Ref = $r;
        ...
        impl ::std::ops::Deref for $n {
            ...
            fn deref(&self) -> &$r {
                unsafe { ::ffi_util::ForeignTypeRef::from_ptr(self.0) }
            }
        }

        impl ::std::ops::DerefMut for $n {
            fn deref_mut(&mut self) -> &mut $r {
                unsafe { ::ffi_util::ForeignTypeRef::from_ptr_mut(self.0) }
            }
        }

        pub struct $r(::ffi_util::Opaque);

        impl ::ffi_util::ForeignTypeRef for $r {
            type CType = $c;
        }
    }
}
